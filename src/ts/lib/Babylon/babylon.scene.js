    ...
    var Scene = (function () {
        ...
        function Scene(engine) {
            ...
            */
             ...
             */
            this.onRenderingGroupObservable = new Observable();
            // Animations
            this.animations = [];
            /**
            ...
            */
            ...
            this._totalMeshesCounter = new PerfCounter();
            this._totalLightsCounter = new PerfCounter();
            this._totalMaterialsCounter = new PerfCounter();
            this._totalTexturesCounter = new PerfCounter();
            this._totalVertices = new PerfCounter();
            this._activeIndices = new PerfCounter();
            this._activeParticles = new PerfCounter();
            this._lastFrameDuration = new PerfCounter();
            this._evaluateActiveMeshesDuration = new PerfCounter();
            this._renderTargetsDuration = new PerfCounter();
            this._particlesDuration = new PerfCounter();
            this._renderDuration = new PerfCounter();
            this._spritesDuration = new PerfCounter();
                    ...
                    if (pickResult.pickedMesh.actionManager) {
                        ...
                        if (pickResult.pickedMesh.actionManager.hasSpecificTrigger(ActionManager.OnLongPressTrigger)) {
                            ...
                            window.setTimeout(function () {
                                var pickResult = that.pick(that._unTranslatedPointerX, that._unTranslatedPointerY, function (mesh) { return mesh.isPickable && mesh.isVisible && mesh.isReady() && mesh.actionManager && mesh.actionManager.hasSpecificTrigger(ActionManager.OnLongPressTrigger); }, false, that.cameraToUseForPointers);
                                if (pickResult.hit && pickResult.pickedMesh) {
                                    if (pickResult.pickedMesh.actionManager) {
                ...
                if (_this.spriteManagers.length > 0) {
                    ...
                    if (pickResult.hit && pickResult.pickedSprite) {
                        if (pickResult.pickedSprite.actionManager) {
                            ...
                            switch (evt.button) {
                                case 0:
                                    pickResult.pickedSprite.actionManager.processTrigger(ActionManager.OnLeftPickTrigger, ActionEvent.CreateNewFromSprite(pickResult.pickedSprite, _this, evt));
                                    break;
                                case 1:
                                    pickResult.pickedSprite.actionManager.processTrigger(ActionManager.OnCenterPickTrigger, ActionEvent.CreateNewFromSprite(pickResult.pickedSprite, _this, evt));
                                    break;
                                case 2:
                                    pickResult.pickedSprite.actionManager.processTrigger(ActionManager.OnRightPickTrigger, ActionEvent.CreateNewFromSprite(pickResult.pickedSprite, _this, evt));
                                    break;
                            }
                            pickResult.pickedSprite.actionManager.processTrigger(ActionManager.OnPickDownTrigger, ActionEvent.CreateNewFromSprite(pickResult.pickedSprite, _this, evt));
                        }
                    }
                }
            };
            this._onPointerUp = function (evt) {
                ...
                if (pickResult.hit && pickResult.pickedMesh) {
                    ...
                    if (pickResult.pickedMesh.actionManager) {
                        ...
                        if (Math.abs(_this._startingPointerPosition.x - _this._pointerX) < ActionManager.DragMovementThreshold && Math.abs(_this._startingPointerPosition.y - _this._pointerY) < ActionManager.DragMovementThreshold) {
                ...
                if (_this._pickedDownMesh && _this._pickedDownMesh !== pickResult.pickedMesh) {
                    _this._pickedDownMesh.actionManager.processTrigger(ActionManager.OnPickOutTrigger, ActionEvent.CreateNew(_this._pickedDownMesh, evt));
                }
                ...
                if (_this.spriteManagers.length > 0) {
                    ...
                    if (pickResult.hit && pickResult.pickedSprite) {
                        if (pickResult.pickedSprite.actionManager) {
                            pickResult.pickedSprite.actionManager.processTrigger(ActionManager.OnPickUpTrigger, ActionEvent.CreateNewFromSprite(pickResult.pickedSprite, _this, evt));
                            if (Math.abs(_this._startingPointerPosition.x - _this._pointerX) < ActionManager.DragMovementThreshold && Math.abs(_this._startingPointerPosition.y - _this._pointerY) < ActionManager.DragMovementThreshold) {
                                pickResult.pickedSprite.actionManager.processTrigger(ActionManager.OnPickTrigger, ActionEvent.CreateNewFromSprite(pickResult.pickedSprite, _this, evt));
                            }
                        }
                    }
                    if (_this._pickedDownSprite && _this._pickedDownSprite !== pickResult.pickedSprite) {
                        _this._pickedDownSprite.actionManager.processTrigger(ActionManager.OnPickOutTrigger, ActionEvent.CreateNewFromSprite(_this._pickedDownSprite, _this, evt));
                    }
                }
            };
            this._onKeyDown = function (evt) {
                if (_this.actionManager) {
                    _this.actionManager.processTrigger(ActionManager.OnKeyDownTrigger, ActionEvent.CreateNewFromScene(_this, evt));
                }
            };
            this._onKeyUp = function (evt) {
                if (_this.actionManager) {
                    _this.actionManager.processTrigger(ActionManager.OnKeyUpTrigger, ActionEvent.CreateNewFromScene(_this, evt));
                }
            };
        ...
        */
        ...
        Scene.prototype.pickWithRay = function (ray, predicate, fastCheck) {
            ...
            return this._internalPick(function (world) {
                ...
                return Ray.Transform(ray, _this._pickWithRayInverseMatrix);
            }, predicate, fastCheck);
        };
